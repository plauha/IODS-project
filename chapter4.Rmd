...
The test results seem to follow the results of the biplot from training data. The hihgest category (4) is mostly correctly classified, except that two instances from class 3 are incorrectly classified to class 4. Classes 3 and 1 get a little bit more mixed with class 1, and even half of all observations belonging to class 1 are incorrectly classified as class 2. However, the results seem fairly good given that the categorization boundaries were quite arbitrary (the distribution of original response variable crime may not support the categorization into four even categories). The model seems to handle the order of the categories pretty well, since no observation is classified more than one class away from the correct one.
...
# run clustering algorithm for 1-10 clusters and save the total sum of squares within cluster
ss = c()
for(k in 1:20){
